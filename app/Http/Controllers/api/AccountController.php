...
class AccountController extends Controller
{
    ...
    public function login(Request $request): Response
    {
        ...
        if (isset($req['captcha'])) {
            if (! captcha_api_check($req['captcha']['value'], $req['captcha']['key'])) {
                return $this->fail(null, $this->code['3005'], 3005);
            }
        }
        /* 用户是否存在 */
        $user = Users::query()->where('email', $req['account'])->whereNull('deleted_at')->first();
        if (! $user) {
            return $this->fail(null, $this->code['3001'], 3001);
        }
        /* 验证密码 */
        if (! Hash::check($req['password'], $user->getAuthPassword())) {
            return $this->fail(null, $this->code['3003'], 3003);
        }
        /* 验证账号禁封状态 */
        if ($user->status === 0) {
            return $this->fail(null, $this->code['3000'], 3000);
        }
        /* 生成令牌 */
